...
describe("buildInfo", () => {
  ...
  it("falls back to the current year when NEXT_PUBLIC_BUILD_DATE is unset", async () => {
    vi.stubEnv("NEXT_PUBLIC_BUILD_DATE", "");

    const buildInfo = await loadBuildInfo();

    expect(buildInfo.year).toBe(String(new Date().getFullYear()));
    expect(buildInfo.date).toBe("");
  });

  it("falls back to the current year when NEXT_PUBLIC_BUILD_DATE cannot be read as a year", async () => {
