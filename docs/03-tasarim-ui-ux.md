...
**Layout (2026-08-28 güncel).** Ana sayfa: `Hero` (`DisplayHeading` + metrik strip, opsiyonel profil fotoğrafı, GitHub/LinkedIn) → tüm projeler (`ProjectList`, live/repo chip) → `ExperienceSummary` → `SkillsStrip` → son yazılar (`PostList`) → `ContactCta`. About sayfasında sticky `AboutSubnav` ve bölüm anchor'ları. Proje/yazı listeleri numaralı editorial satır (`ContentEntryIndex` / `ContentEntryBody`); MDX'te `<Screenshot>` ve `<ProjectMeta>` shortcode'ları hazır. Kapaksız yayın kuralı sürüyor (`content/images/` teslimat bekliyor).
