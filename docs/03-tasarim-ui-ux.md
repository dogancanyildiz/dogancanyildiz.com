...
- **Hareket (karar değişikliği).** Ana sayfadaki hero, yetkinlik şeridi, proje ve yazı listeleri sunucu bileşenine döndü; giriş animasyonları (40 ms stagger, 180 ms) bu dört bölümde yok. Sebep: SSR HTML'de `opacity:0` basılması LCP'yi hidrasyona kilitliyor ve reduced-motion kaçışı bu elemanları kapsamıyordu (denetim, high). LazyMotion özellikleri dinamik import ediliyor; hareket yalnızca contact sayfasında (`fadeUp`). CSS tabanlı bir giriş `.motion-item` kuralı reduced-motion sorgusunun dışına taşınarak geri getirilebilir; **sahibinin görsel onayı bekleniyor.** Kararın 5. maddesindeki "stagger 40 ms, en fazla 4 eleman" sınırı contact için geçerli kalıyor.
...
- **Hâlâ açık:** tarayıcı ekran görüntüsü turu ve yukarıdaki dört görsel onay; contact sayfasının `m.*` elemanları hidrasyona kadar `opacity:0`, `MotionProvider` kök layout'ta; `staggerContainer`/`staggerItem` tüketicisiz; proje kapakları teslimat bekliyor.
