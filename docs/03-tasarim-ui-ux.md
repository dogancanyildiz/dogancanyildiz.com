...
- `src/components/brand/brand-mark.tsx`: export'tan inline React SVG, sunucu bileşeni. Tek bileşen iki temayı da karşılıyor: harfler `currentColor`, blok `fill-primary` (`--primary` açıkta #007041, koyuda #4fcc8d, yani export'un iki dosyasıyla aynı değerler). İki SVG'yi vendor edip birini gizlemek tema sınıfı gelmeden yanlış rengi bir kare gösterirdi. `aria-hidden`; header'daki marka linkinin erişilebilir adı yanındaki isim metni olarak kalıyor. İşaret marka bloğunu yaklaşık 53px genişletti. Cihaz emülasyonuyla ölçüldü (320 ve 375px): satır işareti, ismi ve 44px kontrolleri birlikte taşıyamıyor, üstelik sağ kontrol grubu `min-w-0` olduğu için küçülüp düğmeleri kadraj dışına itiyordu (320px'te işaretsiz de 8px taşma vardı). Karar: sağ grup `shrink-0`, isim 420px altında yalnızca ekran okuyucuya (`max-[419px]:sr-only`, linkin erişilebilir adı değişmiyor), 420px ve üstünde görünür ve gerekirse `truncate` ile üç noktayla kesiliyor. 320, 375, 420, 640 ve 1280'de yatay taşma yok; `tests/accessibility.test.ts` çıplak `sr-only` kullanımını yasaklamaya devam ediyor.
