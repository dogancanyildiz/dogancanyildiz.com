...
- **Hero/header/footer gerçek metin**: `messages/en.json` ve `tr.json`'daki `brand`, `hero`, `contact`, `footer`, `metadata` namespace'leri gerçek içerikle dolduruldu (commit `d2eaf1e`); şablon persona hiçbir yerde kalmadı. Sapma: hero ve header planlanandan farklı yeniden tasarlandı, metrik kartları ve "available for work" rozeti yok, header'da monogram + görünür isim var (plan `sr-only` istiyordu); form konu (subject) alanı kaldırıldı; blog TR-first uygulandı (karar 10 aynen) (`docs/plans/handoffs/faz-4.md` "Plandan sapmalar" madde 18).
