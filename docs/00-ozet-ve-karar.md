...
Durum: Kod tarafı uygulandı (Faz 0-5 main'de, PR #2-#6 ve #31, v0.3.1; 28 Ağustos denetiminin kod bulguları `feature/audit-closure` dalında, 31 Ağustos incelemesinin ve 1-2 Eylül ultrareview'ının bulguları sonraki dallarda kapatıldı; 2 Eylül 3. tur `feature/audit-followups` PR #44 ile `dev`'e merge edildi, T-56 typecheck regresyonu aynı turda kapandı; 2026-09-02 `feature/brand-assets`'te marka paketi, header/footer lockup, paylaş bloğu, sertifika v2, Köklü Hukuk vaka çalışması, yerelleştirilmiş yollar (V-5'in kapanışı) ve kanonik host www kod ve dokümanlara işlendi, apex edge'de www'ye 301 yönlenir), kalan: sahibinin panel/DNS adımları, içerik teslimatları (ilk proje kapağı ve sertifika doğrulama linkleri 2026-09-02'de geldi; Konuşmalar verisi ve kalan beş kapak hâlâ açık), canlı doğrulamalar, `.sh` alan adı kararı ve görsel onaylar (bkz. "marka ve içerik turu" bölümü) · Karar: 2026-08-27 · Güncelleme: 2026-09-02 · Kapsam: dogancanyildiz.com
...
- **Env katmanı hatası:** SMTP_PASSWORD (2026-08-31 öncesi RESEND_API_KEY) Build olarak işaretlenirse image katmanlarına veya build loglarına sızabilir; NEXT_PUBLIC_SITE_URL yalnızca Runtime işaretlenirse client bundle'a hiç gömülmez ve production'da sessizce undefined döner. **Belgelendi, canlıda doğrulanmadı:** Build/Runtime ayrımı docs/deploy/coolify-kurulum.md ve faz-1-manual-checklist.md'de adım adım yazılı; Coolify panelinde fiilen uygulanması ve doğrulanması panel adımları arasında, henüz koşulmadı.
...
**3. tur (`feature/audit-followups`, 2026-09-02).** 5 dosya-ayrık küme (frontend-perf, ui-a11y, backend, seo-i18n-content, test-quality-deps), her biri kendi worktree/dalında opus incelemesiyle çalıştı ve çakışmasız merge edildi; ardından kontrol oturumu üç ek commit'le next-intl mesaj anahtarı tip güvenliğini (T-18) ve bir metin düzeltmesini kapattı. 31 Ağustos incelemesinin V-2..V-18 maddelerinin neredeyse tamamı bu turda kapandı (consent artık geri alınabilir, footer/header/contact/error hedef boyutları 44px/24px'e çıktı, Referer locale'i yalnızca site origin'inden okunuyor, honeypot ve Reply-To sertleştirildi, CSP raporlayıcı sınırlandı, TR kelime sayımı Unicode'a geçti, `verify-docs` artık locale şemasını koruyor); ayrıntı ve kimlikler `audit/acik-kalanlar.md` bölüm 11'de. **Bu turda bulunan bir regresyon, aynı gün kapandı:** T-18'in next-intl `AppConfig` augmentation'ı `Locale` tipini `"en" | "tr"`'ye daraltmıştı ve `npm run typecheck` (tsbuildinfo temizlenmiş) `src/i18n/navigation.ts:21`'de `TS2322` ile kırılıyordu; CI bunu incremental derleme önbelleği yüzünden görmemişti. Düzeltme turu (`1e0e115`) `pathnameForLocale` ve `localePath`'in ikisini de `AppLocale`/`Locale`'e daralttı, iki derleme zamanı regresyon testi ekledi; `npm run typecheck` tsbuildinfo silinerek 0 hatayla geçiyor. PR #44 bu düzeltmeyle birlikte 2026-09-02'de `dev`'e merge edildi (taban commit `c0156c0`). Ayrıntı `audit/acik-kalanlar.md` T-56.

**Kalanlar (2026-09-02, PR #44 sonrası).** Öz olarak 2026-08-28 listesinin devamı: canlı site hâlâ Cloudflare 526; panel adımları (Uptime Kuma kurulumu + `/api/health` monitörü, merkezi Umami'ye site kaydı ve `UMAMI_WEBSITE_ID`, `SMTP_*` env'leri ve `contact@` uygulama parolası, `NEXT_PUBLIC_STATUS_URL`); `.sh` kararı; içerik teslimatları; sahibin görsel onayı istenen değişiklikler (footer'ın 44px hedef boyutuyla uzaması, consent banner'ın artık `role="region"` olması) - bkz. devir notu `docs/plans/handoffs/denetim-takip-2026-09-02.md`. T-56 kapandı, bu listede değil.

## Uygulama durumu (2026-09-02, marka ve içerik turu)

Bu bölüm yukarıdaki 2026-09-02 bölümünün üstünde geçerlidir. Dal `feature/brand-assets` (taban PR #44'ün merge commit'i `c0156c0`), 62 commit, dokuz iş kalemi; her kalemin commit'i, kararı ve gerekçesi devir notunda: [`plans/handoffs/marka-ve-icerik-2026-09-02.md`](./plans/handoffs/marka-ve-icerik-2026-09-02.md).

1. **Marka paketi.** Sahibinin Claude Design export'u siteye bağlandı: DCY işareti + yeşil imleç bloğu (`src/components/brand/brand-mark.tsx`) ve lockup (`brand-lockup.tsx`, header'da imleç yanıp sönüyor, footer'da sabit). Statik ikonlar Next dosya kuralına geçti (`src/app/favicon.ico`, `icon.png`, `apple-icon.png`); eski `ImageResponse` rotaları ve `/favicon.ico -> /icon` yönlendirmesi kalktı. OG kartı sahibinin referans düzenine göre yeniden çizildi (`src/lib/seo/og-layout.tsx`, Geist Mono prompt satırı), blog ve proje sayfaları kendi kartını basıyor, bilinmeyen slug `notFound()` veriyor.
2. **Header.** Lockup (işaret + ayırıcı + isim + "Full Stack · DevOps" mono tagline), 480px altında yalnızca işaret görünüyor. Dil ve tema kontrolleri düzleşti: kapsül ve daire kalktı, TR önce, aktif dil yeşil alt çizgi.
3. **Footer.** Aynı lockup, imleç sabit; availability cümlesi "Yeni projelere açığım, yazın yeter." oldu; `brand.monogram` anahtarı silindi.
4. **Paylaş bloğu.** Yazı ve proje sayfalarının sonunda kart, X/LinkedIn/WhatsApp/e-posta linkleri ve "bağlantıyı kopyala" düğmesi (`src/components/sections/share-card.tsx`); RSS öğeleri artık kendi kartını `media:content` ile taşıyor.
5. **Yazım ve yetkinlikler.** "Full Stack" tiresiz her yerde; Tailwind CSS ve C# / ASP.NET MVC (.NET ikonuyla) yetkinlik listesine eklendi.
6. **Sertifikalar v2.** On üç kayıt, verene göre gruplu, Credly/Hackviser doğrulama linkleri ve yerel rozet görselleriyle (`public/images/badges/`); satır artık tıklanınca büyüyen bir önizleme açıyor (`credential-preview.tsx`), iki sütunlu düzen, verenin kendi etiketlerinden anahtar kelime satırı, eğitim satırlarında okul amblemleri (`public/images/schools/`).
7. **Köklü Hukuk vaka çalışması.** Altıncı proje, ilk kapaklı yayın (`content/images/koklu-hukuk-cover.jpg`), outcome tarihsiz teslim cümlesiyle yazıldı; Hubit canlı linki eklendi; diller cümlesi İngilizceyi "teknik işin günlük dili" olarak tanımlıyor.
8. **Yerel yollar ve çeviri başına slug (V-5'in kapanışı).** TR tarafı tamamen Türkçe (`/projeler/<tr-slug>`, `/yazilar`, `/yazilar/<tr-slug>`), her çeviri kendi slug'ını taşıyor (`translationKey` frontmatter alanı eşliyor, `slug` değil), üç 308 tablosu (öneksiz, `/en`, `/tr`) eski adresleri tek atlamada karşılıyor, dil değiştirici `useParams` + `buildTranslationMap`'e geçti, feed guid `translationKey` tabanlı tag URI oldu. Detay: [04-i18n.md](./04-i18n.md).
9. **Kanonik host www.** `NEXT_PUBLIC_SITE_URL`, security.txt ve dokümanlar `www.dogancanyildiz.com`'a sabitlendi; apex edge'de buna 301 ile yönlenecek.

**Kapılar (bu bölümün yazıldığı HEAD).** `tsc --noEmit` 0 hata (tsbuildinfo silinerek), `eslint` 0 uyarı, `npm test` 81 dosya / 1258 test, `npm run format` temiz, `npm run build` 44 sayfa (yalnızca `/api/contact`, `/api/csp-report`, `/api/health` dynamic), `npm run verify:routes` 41 içerik rotası (6 proje + 3 yazı, iki locale), `npm run verify:docs` 21 dosya, sitemap 30 URL.

**Kalan (panel dışı, sahibinde).** Görsel onaylar: header lockup, imleç animasyonu, düz dil/tema kontrolleri, footer uzunluğu, paylaş bloğu, sertifika önizleme kalıbı, okul logoları (Konya Teknik'in koyu temada zayıflaması bilinçli kabul), Köklü Hukuk kapağı. İçerik teslimatları hâlâ açık: Konuşmalar verisi, kalan beş proje kapağı (Köklü Hukuk hariç). Panel adımları değişmedi (bkz. yukarıdaki liste).
...
| Backend / içerik altyapısı | Velite + MDX, contact sertleştirme, status vitrini | Yüksek | 05-backend-icerik-ve-servisler.md | Uygulandı: Velite Faz 4, contact sertleştirme Faz 0 + 2026-08-28 kapanışı; **2026-08-30:** izleme Uptime Kuma'ya, ölçüm merkezi Umami'ye (`umami.dravcore.com`) taşındı, Gatus ve `infra/` silindi, Systems paneli ince; **2026-08-31:** Resend yerine Mailcow SMTP (`docs/deploy/mailcow-smtp.md`); kalan: Kuma kurulumu, Umami'ye site kaydı, SMTP env'leri |
...
- Self-hosted SMTP (Stalwart/Postal): deliverability riski tek bir contact formu için orantısız görülmüştü. **Karar değişikliği (2026-08-31):** sahibinin zaten çalışan Mailcow sunucusu olduğu ve form yalnızca sahibinin kutusuna teslim ettiği için Resend kaldırıldı, gönderim Mailcow SMTP'ye geçti; yeni bir posta yazılımı kurulmadı, itiraz edilen 'sıfırdan SMTP işletmek' değildi.
