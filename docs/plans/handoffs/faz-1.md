...
- Faz 2 route'ları dinamikleştirirken `/api/health`'in `200 {"status":"ok"}` ve `no-store` sözleşmesini bozmamalı: Coolify health check, Dockerfile HEALTHCHECK ve Gatus (Faz 5) aynı yola bakıyor.
