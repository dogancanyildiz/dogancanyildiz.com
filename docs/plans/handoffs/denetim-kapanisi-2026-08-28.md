...
## Ek not (2026-09-02): ultrareview kapanışı ve bağımlılıklar

PR #37 (gözlemlenebilirlik panele, Mailcow SMTP, dil değiştirici blocker düzeltmesi, doküman tazeleme) 2026-08-30'da squash ile dev'e girdi. Merge edilen diff üzerinde koşulan bulut incelemesi (ultrareview) 6 bulgu verdi ve tamamı PR #39 ile kapandı (2026-09-01, `bd17329`):

- **Gerçek hata:** `NEXT_PUBLIC_STATUS_URL` Build değişkeni olarak dokümante edilmişti ama Dockerfile'ın builder aşamasında `ARG`/`ENV` tanımı yoktu; Coolify değeri geçse bile Docker onu atıyordu ve Systems panelindeki Kuma status linki production'da hiç görünemezdi. ARG/ENV eklendi; builder aşamasındaki beş opsiyonel public build değişkeninin ARG/ENV çiftini zorunlu kılan bir test bu hata sınıfını kalıcı kilitledi.
- **Bayat uçlar:** `docker-compose.yml` ve üç sızıntı bekçisi test listesi `RESEND_API_KEY` yerine `SMTP_*`'a çevrildi (gelecekte `SMTP_PASSWORD` bir Dockerfile veya workflow'a sızarsa test düşer); contact route'un zaman aşımı yorumu kaldırılan idempotency anahtarını vaat etmeyi bıraktı; health/log yorumları ve README alarm tüketicisi olarak Uptime Kuma'yı söylüyor; `coolify-kurulum.md` silinen `resend-domain.md` yerine `mailcow-smtp.md`'ye bağlanıyor, `GATUS_URL` satırı ve `re_` prefix doğrulaması kalktı; yol haritasının Faz 1 açık maddeleri SMTP/Mailcow gerçeğine güncellendi.

Ders: yerel kapılar (lint, tsc, vitest) ve yedi CI check'i imaj build zincirindeki bu sınıf hatayı göremiyordu; Dockerfile ARG'ları artık testle bağlandı. Ayrıca Dependabot #38'i kapatıp #40 olarak yeniden açtı; grup (lucide-react 1.37.0, next-intl 4.14.1, simple-icons 16.29.0, zod 4.5.4) yerel doğrulama (tsc, 979 test, production build) sonrası merge edildi (2026-09-02). next-intl 4.14'ün tek özelliği `useExtracted` iyileştirmeleri; bu repo klasik mesaj kataloğu kullandığından etkilenmiyor.
