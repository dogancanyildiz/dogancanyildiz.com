...
- **EN sayfalarda og:image `/en/` prefix'li** (`https://dogancanyildiz.sh/en/opengraph-image/default?<hash>`), 307 ile prefix'siz adrese gidiyor; TR tarafı doğrudan 200. Next dosya tabanlı og image'ı `[lang]` segmentinden türettiği için oluyor. Yönlendirme izleyen crawler'lar için sorun değil; Faz 3 `opengraph-image.tsx`'i yeniden yazarken ya `openGraph.images`'i `localeUrl(lang, "/opengraph-image/default")` ile override eder (hash query'si düşer) ya da hop'u kabul edip not eder.
- **Alt sayfalarda `og:url` ve `og:title` layout'tan miras** (`/about` için `og:url` `https://dogancanyildiz.sh`, `og:title` "Alex Chen Portfolio"; canonical doğru). Plan Step 8'in bilinçli seçimi; paylaşım kartlarında alt sayfa kimliği istenirse her `generateMetadata`'ya `openGraph: { url: localeUrl(lang, path), title, description }` eklenir.
...
| `src/app/[lang]/layout.tsx` | Kök layout: `generateStaticParams`, `dynamicParams = false`, `generateMetadata` (`metadataBase`, title template, `openGraph` locale/alternateLocale), `setRequestLocale`, `<html lang={lang}>`, `NextIntlClientProvider` |
| `src/app/[lang]/*/page.tsx` | Her sayfa: `generateStaticParams`, `hasLocale` kontrolü, `setRequestLocale(lang)`, `generateMetadata` içinde `alternates: buildAlternates(lang, path[, localesForProject(slug)])` |
...
| `src/app/[lang]/opengraph-image.tsx` | `generateImageMetadata` per-locale `alt` üretir, boş params için `defaultLocale` fallback'i; `runtime` export'u yok |
| `src/lib/seo/locale-url.ts` | `localePath(locale, pathname)`, `localeUrl(locale, pathname)`, `buildAlternates(locale, pathname, availableLocales?)` (canonical + `languages` + `x-default`) |
...
- **`opengraph-image.tsx`'i yeniden yazarken** `generateImageMetadata`'nın boş params ile ilk çağrısını unutmayın (`hasLocale` fallback'i kalmalı), `runtime` export'u eklemeyin (edge kullanılmıyor), `metadataBase` layout'ta kalmalı.
...
| 4 metadata, canonical, hreflang | sonnet | `926a4bd` | yok | yok | Alt sayfalarda `og:url`/`og:title` layout'tan miras (plan seçimi); `locale-url.test.ts` env stub temizliği; `project-locales.ts` testsiz; beşinci test `env.test.ts` ile çakışık; `/` canonical'ında sondaki `/` düşüyor (eşdeğer) |
