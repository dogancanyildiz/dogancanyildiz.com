...
- [ ] Bölüm 4, env katmanları: `NEXT_PUBLIC_SITE_URL=https://dogancanyildiz.sh` **Build Variable**; `RESEND_API_KEY`, `CONTACT_EMAIL=me@dogancanyildiz.com`, `FROM_EMAIL=contact@dogancanyildiz.sh`, `TRUST_CF_CONNECTING_IP=false`, `GATUS_URL` (boş, Faz 5) **Runtime only**. Dockerfile `NEXT_PUBLIC_SITE_URL` için `https://dogancanyildiz.sh` varsayılanı taşır; Build Variable yine de açıkça set edilir ki preview ve staging'de niyet belli olsun.
...
- [ ] Bölüm 4: Custom Labels `traefik.http.routers.portfolio.middlewares=security-headers@file,compress@file`; `buffering` eklenmez. Doğrulama: `curl -sI https://dogancanyildiz.sh/` -> `strict-transport-security: max-age=31536000; includeSubDomains`, `x-powered-by` yok, `content-encoding` `br` veya `zstd`.
- [ ] Bölüm 5a, ufw: `ADMIN_IPV4` değerini `curl -s https://api.ipify.org` ile al; Cloudflare 22 bloğu + OpenSSH + admin = 24 kural. Doğrulama, allowlist dışındaki bir ağdan: `curl -sS --max-time 8 --resolve dogancanyildiz.sh:443:ORIGIN_IPV4 https://dogancanyildiz.sh/api/health` -> `curl: (28)` veya `curl: (7)` (Bitti kriteri 9). Gövde dönerse kısıt çalışmıyor.
- [ ] **Ancak bölüm 2 VE bölüm 5 tamamlandıktan sonra**: Coolify -> Environment Variables -> `TRUST_CF_CONNECTING_IP=true`, redeploy. Doğrulama: `CF-Connecting-IP: 203.0.113.$i` başlıklı 6 istekte son yanıt `429` (dokümanın bölüm 2 döngüsü). Sıra ters çevrilirse rate limit atlanabilir hale gelir.
- [ ] `ADMIN_IPV4` değiştikçe ufw kuralı güncellenir (admin adresi ve tarihi `docs/plans/handoffs/faz-1.md` "Açık kalanlar" bölümüne yazılır).
