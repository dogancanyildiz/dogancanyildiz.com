...
Durum: Uygulandı (Faz 2 ve Faz 4; 2026-08-28 denetim kapanışı; **2026-08-30, PR #36:** TR varsayılan, x-default ve JSON-LD kimlikleri TR köküne, eski EN yolları 308; 2026-09-02 3. tur: TR kelime sayımı Unicode'a geçti, çeviri paritesi testle kilitlendi; 2026-09-02 marka varlıkları: sayfa başına OG kartı, statik ikonlar), kalan: Search Console ve hreflang canlı doğrulaması · Karar: 2026-08-27 · Güncelleme: 2026-09-02 · Kapsam: dogancanyildiz.com
...
- **`opengraph-image.tsx` ve `icon.tsx` yeniden yazıldı (Faz 3/4), sonra marka varlıklarıyla değiştirildi (2026-09-02).** "Alex Chen"/"P" kalıntısı kod tabanında sıfır (`tests/no-template-residue.test.ts` bunu garanti ediyor). Güncel durum bir alttaki marka bölümünde.
...
- **OG görseli düzeldi (denetimin kritik bulgusu).** Rota üretimde 500 veriyordu: satori değişken (fvar/gvar) font okuyamıyor. `scripts/vendor-fonts.mjs` fontTools ile statik TTF instance'ları üretiyor (`public/fonts/og/*.ttf`), rota bunları yüklüyor; `tests/og-image.test.ts` gerçek render yapıp PNG imzasını doğruluyor. Eski `public/fonts/og/*.woff` kopyaları ölü, temizlendi.
- **OG görseli ve ikonlar marka varlıklarına geçti (2026-09-02).** Düzen sahibinin referansına göre yeniden yazıldı (detay: [03-tasarim-ui-ux.md](./03-tasarim-ui-ux.md)); ortak kod `src/lib/seo/og-layout.tsx`. Üç rota var: `[lang]/opengraph-image.tsx` (kimlik kartı), `[lang]/blog/[slug]/opengraph-image.tsx` ve `[lang]/projects/[slug]/opengraph-image.tsx` (sayfa başlığıyla). Sayfa kartının metadata tarafı asıl dikkat isteyen kısım: Next `openGraph`'ı miras almıyor, **değiştiriyor**, yani dosya kuralı tek başına bırakılsa iki detay sayfası da kendi görseli dururken jenerik kartı ilan ederdi. `buildOpenGraph` opsiyonel `imagePath` alıyor, `ogImagePathFor` bunu sayfa yolundan kuruyor, iki `page.tsx` geçiriyor; `alternates` ve canonical'a dokunulmadı. İki sayfa rotası slug bulunamazsa `notFound()` çağırıyor: varsayılan davranışta sayfası 404 veren bir slug için bile 200 marka kartı üretiliyordu ve istenen yol kartın üstündeki prompt satırına birebir yazılıyordu. Kontrol handler'da, çünkü `dynamicParams = false` metadata görsel rotalarında gerçek kartları da 404'e düşürüyor (manifest'te somut yol yok). Alt metin de sayfaya özel: `metadata.ogAltPage` anahtarı başlığı alıyor, kimlik kartının alt metni yalnızca `[lang]` rotasında kalıyor. `next start` üzerinde doğrulandı: `/blog/<slug>/opengraph-image/default` ve `/projects/<slug>/opengraph-image/default` iki locale'de, önekli ve öneksiz, 200 `image/png`. İkonlar üretilen route değil statik dosya (`src/app/favicon.ico`, `icon.png`, `apple-icon.png`); build HTML'inde `<link rel="icon">` iki kez (ico 48x48, png 192x192) ve `<link rel="apple-touch-icon">` bir kez çıkıyor.
