...
4. **`robots.ts` gerçek domain'i kullanır** (`https://dogancanyildiz.sh`), `example.com` fallback'i kaldırılır.
...
7. **`.sh` birincil domain kalır**, `.com` yalnızca 301 hedefi (öneri, sahibinin kesin onayı bekliyor: bkz. [11-acik-sorular.md](./11-acik-sorular.md) soru 5); SEO cezası yok, marka/güven notu var (Gerekçe'de). İki domain'de aynı içeriği aynı anda yayınlamak (`.sh` ve `.com`'un ikisinin de canlıda tam site sunması) **kesinlikle yapılmaz**, çünkü bu duplicate content yaratır ve `alternates.canonical` tek bir domain'e işaret etmek zorundadır. Sahibi ileride `.com`'u birincil yapmak isterse yalnızca yönlendirmenin yönü ve `NEXT_PUBLIC_SITE_URL` değişir, `generateMetadata`/`sitemap.ts`/`robots.ts` kodu dokunulmadan kalır.
...
|---|---|---|---|
| Ana sayfa | `https://dogancanyildiz.sh/` | `https://dogancanyildiz.sh/tr` | `https://dogancanyildiz.sh/` |
| About | `https://dogancanyildiz.sh/about` | `https://dogancanyildiz.sh/tr/about` | `https://dogancanyildiz.sh/about` |
| Projects | `https://dogancanyildiz.sh/projects` | `https://dogancanyildiz.sh/tr/projects` | `https://dogancanyildiz.sh/projects` |
| Blog yazısı (iki dilde) | `https://dogancanyildiz.sh/blog/<slug>` | `https://dogancanyildiz.sh/tr/blog/<slug>` | `https://dogancanyildiz.sh/blog/<slug>` |
| Blog yazısı (yalnız TR) | girmez | `https://dogancanyildiz.sh/tr/blog/<slug>` | girmez |
...
| `.sh` uzantısının teknik olmayan ziyaretçide dönüşüm tereddütü yaratması | Bu bir SEO riski değil, dönüşüm riski; ölçüm Faz 5'teki Umami analytics'e bırakılır | [08-icerik-stratejisi.md](./08-icerik-stratejisi.md) |
...
- [06-devops-ve-deploy.md](./06-devops-ve-deploy.md): `.com -> .sh` 301'i, Coolify Build/Runtime env ayrımı
